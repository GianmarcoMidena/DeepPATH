...
def inputs(dataset, batch_size=None, num_preprocess_threads=None):
  ...
  with tf.device('/cpu:0'):
    images, labels, all_filenames, filename_queue = batch_inputs(
        dataset, batch_size, train=False,
        num_preprocess_threads=num_preprocess_threads,
        num_readers=1)

  return images, labels, all_filenames, filename_queue
...
def parse_example_proto(example_serialized):
  ...
  if FLAGS.mode == '0_softmax':
    ...
    feature_map = {
        'image/encoded': tf.io.FixedLenFeature([], dtype=tf.string,
                                            default_value=''),
        'image/class/label': tf.io.FixedLenFeature([FLAGS.ClassNumber+1], dtype=tf.int64,
...
def batch_inputs(dataset, batch_size, train, num_preprocess_threads=None,
  ...
  with tf.compat.v1.name_scope('batch_processing'):
    ...
    examples_per_shard = nbr_slides
    FLAGS.num_examples = nbr_slides
    ...
    for thread_id in range(num_preprocess_threads):
      ...
      image_buffer, label_index, bbox, _, filename_extract = parse_example_proto(
          example_serialized)
    ...
    images, label_index_batch, all_filenames2 = tf.compat.v1.train.batch_join(
        ...
        capacity=3 * num_preprocess_threads * batch_size * (round(nbr_slides/batch_size)+1))

    #all_filenames2 = tf.train.batch_join( [all_filenames], batch_size=batch_size, capacity= num_preprocess_threads * batch_size)
    ...
    #all_filenames2 = tf.cast(all_filenames2, tf.string)
