...
class FCTest(tf.test.TestCase):
  ...
  def testCreateFC(self):
    height, width = 3, 3
    with self.test_session():
      inputs = tf.random.uniform((5, height * width * 3), seed=1)
...
class DropoutTest(tf.test.TestCase):
  ...
  def testCreateDropoutNoTraining(self):
    height, width = 3, 3
    with self.test_session():
      images = tf.random.uniform((5, height, width, 3), seed=1, name='images')
...
class FlattenTest(tf.test.TestCase):
  ...
  def testFlatten3D(self):
    height, width = 3, 3
    with self.test_session():
      images = tf.random.uniform((5, height, width), seed=1, name='images')
      output = ops.flatten(images)
      self.assertEquals(output.get_shape().num_elements(),
                        images.get_shape().num_elements())
      self.assertEqual(output.get_shape()[0], images.get_shape()[0])
  ...
  def testFlattenBatchSize(self):
    ...
    with self.test_session() as sess:
      images = tf.random.uniform((5, height, width, 3), seed=1, name='images')
      inputs = tf.compat.v1.placeholder(tf.int32, (None, height, width, 3))
...
class BatchNormTest(tf.test.TestCase):
  ...
  def testCreateVariables(self):
    ...
    with self.test_session():
      ...
      moving_mean = tf.compat.v1.moving_average_variables()[0]
      moving_variance = tf.compat.v1.moving_average_variables()[1]
  ...
  def testUpdateOps(self):
    ...
    with self.test_session():
      images = tf.random.uniform((5, height, width, 3), seed=1)
      ops.batch_norm(images)
      update_ops = tf.compat.v1.get_collection(ops.UPDATE_OPS_COLLECTION)
  ...
  def testReuseVariables(self):
    ...
    with self.test_session():
      ...
      moving_vars = tf.compat.v1.get_collection('moving_vars')
  ...
  def testReuseUpdateOps(self):
    ...
    with self.test_session():
      ...
      self.assertEquals(len(tf.compat.v1.get_collection(ops.UPDATE_OPS_COLLECTION)), 2)
      ops.batch_norm(images, scope='bn', reuse=True)
      self.assertEquals(len(tf.compat.v1.get_collection(ops.UPDATE_OPS_COLLECTION)), 4)
  ...
  def testCreateMovingVars(self):
    ...
    with self.test_session():
      ...
      moving_mean = tf.compat.v1.get_collection('moving_vars',
                                      'BatchNorm/moving_mean')
      self.assertEquals(len(moving_mean), 1)
      self.assertEquals(moving_mean[0].op.name, 'BatchNorm/moving_mean')
      moving_variance = tf.compat.v1.get_collection('moving_vars',
  ...
  def testEvalMovingVars(self):
    ...
    with self.test_session() as sess:
      ...
      init_assigns = [tf.compat.v1.assign(moving_mean, expected_mean),
                      tf.compat.v1.assign(moving_variance, expected_var)]
