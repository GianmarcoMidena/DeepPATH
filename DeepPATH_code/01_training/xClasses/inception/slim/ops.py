...
def batch_norm(inputs,
  ...
  with tf.compat.v1.variable_scope(scope, 'BatchNorm', [inputs], reuse=reuse):
    ...
    moving_collections = [moving_vars, tf.compat.v1.GraphKeys.MOVING_AVERAGE_VARIABLES]
    ...
    if is_training:
      ...
      mean, variance = tf.nn.moments(x=inputs, axes=axis)
...
def conv2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope, 'Conv', [inputs], reuse=reuse):
    ...
    weights = variables.variable('weights',
                                 ...
                                 trainable=trainable,
                                 restore=restore)
    conv = tf.nn.conv2d(input=inputs, filters=weights, strides=[1, stride_h, stride_w, 1],
                        padding=padding)
    if batch_norm_params is not None:
      ...
      bias_initializer = tf.compat.v1.constant_initializer(bias)
  ...
  with tf.compat.v1.variable_scope(scope, 'FC', [inputs], reuse=reuse):
    ...
    if batch_norm_params is not None:
      ...
      biases = variables.variable('biases',
                                  ...
                                  trainable=trainable,
                                  restore=restore)
      outputs = tf.compat.v1.nn.xw_plus_b(inputs, weights, biases)
    if activation:
      outputs = activation(outputs)
    return outputs
...
def one_hot_encoding(labels, num_classes, scope=None):
  ...
  with tf.compat.v1.name_scope(scope, 'OneHotEncoding', [labels]):
    ...
    onehot_labels = tf.compat.v1.sparse_to_dense(
...
def max_pool(inputs, kernel_size, stride=2, padding='VALID', scope=None):
  ...
  with tf.compat.v1.name_scope(scope, 'MaxPool', [inputs]):
    kernel_h, kernel_w = _two_element_tuple(kernel_size)
    stride_h, stride_w = _two_element_tuple(stride)
    return tf.nn.max_pool2d(input=inputs,
...
def avg_pool(inputs, kernel_size, stride=2, padding='VALID', scope=None):
  ...
  with tf.compat.v1.name_scope(scope, 'AvgPool', [inputs]):
    kernel_h, kernel_w = _two_element_tuple(kernel_size)
    stride_h, stride_w = _two_element_tuple(stride)
    return tf.nn.avg_pool2d(input=inputs,
                          ksize=[1, kernel_h, kernel_w, 1],
                          strides=[1, stride_h, stride_w, 1],
                          padding=padding)
...
def dropout(inputs, keep_prob=0.5, is_training=True, scope=None):
  ...
  if is_training and keep_prob > 0:
    with tf.compat.v1.name_scope(scope, 'Dropout', [inputs]):
      return tf.nn.dropout(inputs, 1 - (keep_prob))
  else:
    return inputs
